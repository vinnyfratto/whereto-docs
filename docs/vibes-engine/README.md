...
Related: [The Hawaii Sub-Region Anomaly](hawaii-subregion-anomaly.md), resolved 2026-08-06,
for the one time the repo and the database held two different answers and what it took to
reconcile them, and [Deferred Fixes](deferred-fixes.md) for the running list of problems a
pass found and deliberately left alone.
...
- 809 scored vibes and 7,936 scored destination rows across 32 sub-regions, all as local
  markdown in `Content/VibesEngine/`.
- 809 long-form blogs, one per vibe per sub-region.
- A public-facing review page (wheretotrips.com/vibes-engine) for browsing the content.
- Live Supabase tables. `vibe_destination_rankings` holds the scored rows and `vibe_blogs`
  holds the blog bodies, both seeded from the markdown by `scripts/seed-vibe-rankings.js`
  and `scripts/seed-vibe-blogs.js`.
- A live read path. `fetchDestinationBlogs()` in `src/lib/vibeBlogs.ts` resolves a
  destination's sub-region from its highest-scoring ranking row, then loads the matching
  `vibe_blogs` row, which is what renders the vibe content on the Explore screen.

What is **not** done yet: wiring vibe rankings into the app's actual recommendation and
search scoring, and connecting `hotel_tag` to hotel search filtering. The in-app "Wander
with Vibes" and "WhereTo: Discover" search flows still run on the smaller
`src/data/vibesCanonical.ts` taxonomy, independent of the ranking data.

Note that because the read path resolves a sub-region from the highest-scoring row, a
destination with rows under two sub-regions will silently resolve to whichever scores
higher. That is what went wrong with Hawaii, and it is worth remembering before any future
destination is scored in two places. See
