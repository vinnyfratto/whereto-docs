...
- PostHog error tracking needs **two** switches: the project setting *and*
  ...
  **Superseded 2026-09-14:** error monitoring moved to Sentry
  (`docs/decisions/0011-sentry-error-monitoring.md`), which also sees native
  crashes, hangs and the Edge Functions. The SDK-side `errorTracking` flag was
  removed from `src/utils/analytics.ts`, so PostHog no longer captures
  exceptions whatever the project toggle says. Do not turn it back on: every JS
  error would then be reported twice.
...
- PostHog error tracking — **both switches on** as of 2026-08-10, see the
  PostHog console configuration section above. **Superseded 2026-09-14** by
  Sentry (ADR-0011); the SDK flag is gone and Sentry's build plugin handles the
  source map and debug symbol upload.
...
5. ~~**dSYM / mapping-file upload** into the EAS build pipeline, so captured
   exceptions symbolicate.~~ Moot since 2026-09-14: exceptions go to Sentry,
   whose Expo plugin uploads source maps and debug symbols during EAS Build.
