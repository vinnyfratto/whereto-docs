...
> **Note added 2026-09-17.** The §7 list below is still live, but read it against today's
> code: the wizard components it audits (`PassengerWizard`, `ReviewConfirmSheet`) were
> deleted with the last of Duffel on 2026-09-17, and flight checkout now runs through
> `FlightPrebookView` and `liteapi-flights`. The gaps themselves (payment capture, seats,
> `attachServices`, the confirmation email) are unchanged; only the file names moved. One
> item, a stale Duffel disclaimer line, was dropped because the file holding it is gone.
...
**Tier 2 — parity & trust:**
5. Trip protection / insurance offer.
6. Promo/coupon code + any loyalty concept (same opportunity as hotels' `suggestedSellingPrice`/member-pricing gap — consider solving both together).
7. Real payment UI is Tier 1 above; billing-address capture rides along with it.

**Tier 3 — revenue & differentiation:**
8. Price-drop protection.
9. Pay-over-time (Affirm-style) — likely post-launch, same as June.
10. Package (flight + hotel) bundle upsell — flagged in the hotel teardown as a differentiator WhereTo is uniquely positioned to build (both LiteAPI integrations already exist); applies equally from the flight side.
