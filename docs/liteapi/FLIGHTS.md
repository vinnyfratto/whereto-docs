...
### 7a. Observed against a live prebook (2026-07-29) — corrections to the above

Captured from LiteAPI's own API testbed (JFK→LHR, 1 adult, `usePaymentSdk: true`).
Where this disagrees with the reference page, **this section is what is actually
on the wire**.

- **`publishableKey` is ABSENT, not null.** The key is simply not in the
  response. The Stripe publishable key therefore has to come from Nuitee out of
  band — nothing in the prebook payload can supply it.
- **Passenger type has two spellings and the testbed uses the other one.** The
  reference page documents `passengerType` (int 0/1/2); the testbed sends
  `"type": "ADT"` and no `passengerType`. The response echoes **neither**, so
  which one is read is not observable. WhereTo sends **both** (`toLitePassenger`
  in `src/utils/flightBooking.ts`). Matters the moment children/infants exist.
- **`servicesAttachable.groups[]` has NO group-level `available` field.** Groups
  are `{ category, label, services }`. §7's mention of `available` is wrong.
- **Blocked seats ARE returned, and they are all priced 0.** Availability is
  per-seat at `metadata.seat.available`. 13 of 46 seats came back
  `available: false`, every one at `amount: 0` — so anything keying "free seat"
  off price alone renders exactly the unsellable seats as the most attractive
  ones. `seatIsSellable()` in `src/components/booking/SeatMap.tsx` is the guard.
- **`paymentTypes` came back `["TRANSACTION_ID","ACC_CREDIT_CARD","WALLET"]` —
  no `CREDIT`.** Further evidence the list is not to be trusted: it advertises
  the one value book rejects (45009) and omits one it accepts.
- **The prebook `booking` block is thinner than §7 implies.** Real keys:
  `timestamp`, `journey`, `passengers`, `order.reference`, `contact`, `pricing`.
  There is **no** `bookingId`, `bookingRef`, `status` or `paymentStatus` — so the
  edge function's `provisional` mapping resolves to nulls for all four.
  `order.reference` does carry a **real airline PNR issued before any payment**
  (`orderId: "ER9INL"`, `airlinePnr: "FB3FIF"`).
- **`servicesAttachable.expiresAt` = prebook + exactly 15:00**, confirming the
  PREBOOKED TTL in §3.
- **`journey.baggage.included[]` can be `personal` only** (a backpack), with the
  carry-on sold separately in `baggage.paid[]` at $38.34. Folding `personal` into
  `carry_on` tells the traveller they have a carry-on they must pay for at the
  gate — see `includedBaggage()` in `supabase/functions/liteapi-flights/index.ts`.
  Note `baggage.paid[]` and the `servicesAttachable` baggage group price the same
  bag differently ($57.50 vs $44.73); the attachable one is what can be bought.
- **The book request is confirmed identical to ours:**
  `{ prebookId, payment: { method: "TRANSACTION_ID", transactionId } }`, and the
  prebook request carries **no `payment` object**, so `descriptorSuffix` /
  `paymentMethodConfiguration` are genuinely optional.
- **Still unresolved:** the testbed's book request is a template with a literal
  `"<transactionId from prebook>"` placeholder and returns no booking response,
  so it does **not** tell us whether a Stripe confirm happens between prebook and
  book. The `55099` question in §10 remains open.

---

### 7b. Baggage scope: the catalog is per COUPON (2026-09-07)

**A baggage service carrying a `segmentKey` covers THAT SEGMENT ONLY.** There is no
direction-level baggage product and no `scope` field: `FlightBookedService.segmentKey`
is "the flight segment this service applies to", singular. Confirmed by LiteAPI support
and verified on the wire.

Live pull, ORD→DUB→MAD round trip, Aer Lingus "Saver", 1 adult. `servicesAttachable`
returned **four** `Standard Check In Baggage 20kg` services for what is one suitcase:

| segmentKey | coupon | direction | price |
|---|---|---|---|
| `11ff479d` | ORD→DUB | OUTBOUND | $67.25 |
| `2b220bb4` | DUB→MAD | OUTBOUND | $48.90 |
| `2daaa2ea` | MAD→DUB | INBOUND  | $48.90 |
| `2d2156fe` | DUB→ORD | INBOUND  | $67.25 |

So to sell "one checked bag, Chicago to Madrid" you must attach BOTH outbound
services. Attaching only the first buys a bag for ORD→DUB and nothing beyond the
connection. Verified end to end on prebook `01a07e1e-2347-7cf5-9203-2480383cc43c`:
base $601.93, attach all four → **$834.23** ($601.93 + $232.30), and `bookedServices`
came back with four entries each against its own `segmentKey`.

The UI must NOT mirror this shape. No airline or OTA sells a bag per layover, so the
Bags step collapses to one row per direction at the summed price and expands to one
`selectedServices` entry per coupon on attach — see `bagBundles` in
`src/components/booking/FlightPrebookView.tsx`. Direction is derived from the slice a
`segmentKey` sits in, because `mapSegment` drops LiteAPI's `direction` field while
`mapSlices` groups by it.

- **`journey.baggage.paid[]` is reference data, not a catalog.** Same prebook: 8 entries,
  no `segmentKey`, and prices matching none of the attachable ones ($31.92 / $48.47 /
  $38.96 / $37.68 for the same 20kg bag sold at $67.25 / $48.90). Its 25kg tier never
  appears in `servicesAttachable` and therefore cannot be sold at all. Confirmed expected
  behaviour. This is the same mismatch §7a noted for JFK→LHR, so it is systematic.
- **If it is not in `servicesAttachable`, there is no `serviceId` and it cannot be sold.**
- **Ancillary availability is supplier-dependent.** On the same day, United, Turkish,
  Royal Air Maroc and Lufthansa returned ZERO attachable bags on connecting itineraries;
  only Aer Lingus returned any. The per-coupon rule does not change by supplier.
- **Read back what was actually bought** from `booking.bookedServices` (prebook) or
  `GET /flights/bookings/{bookingId}/services` (post-book) rather than trusting our own
  arithmetic — those carry the prices that were charged.
...
> ⚠️ **That idempotency does NOT extend to a FAILED book (observed 2026-07-28).**
> A book that fails puts the prebook into a terminal state, and every further
> attempt against the same `prebookId` returns:
> ```
> 409 {"error":{"code":45038,"description":"Booking is in terminal state: failed",
>      "key":"bodyRequest","message":"conflict: invalid status transition"}}
> ```
> So "retry the book" is only safe after a *transport* failure where the request
> may never have arrived. After any provider rejection the only valid recovery is
> a **new verify + prebook**. WhereTo marks the prebook dead on failure and the
> retry affordance re-locks a fresh fare rather than re-posting the old one.
>
> **Observed booking-stage failure, sandbox:**
> ```
> 500 {"error":{"code":55099,"description":"Failed to create booking",
>      "message":"unable to process booking request"}}
> ```
> **Reproduced across unrelated offers 2026-07-28** (MIA→YYZ→LIS $5,069.23 and
> LGA→YUL→FRA→OSL $4,312.66, both healthy prebooks with `txn` + `secret` and
> `paymentTypes: TRANSACTION_ID, ACC_CREDIT_CARD, WALLET`).
>
> ### 55099 = the sandbox cannot ticket REAL carriers. Settled. (2026-07-29)
>
> Resolved from the Nuitee dashboard's own **Flight Reservations** page
> (`connect.nuitee.com/flights/`), which exposes the one column the API never
> returns: the operating carrier. Across 14 bookings the split is total.
>
> | Booking ref | Route | Carrier | Status | Payment |
> |---|---|---|---|---|
> | FH-267-LAPCCKWP | AUS→MRS | **Nuitée Air** | CONFIRMED | Paid |
> | FH-267-NL7PAQR6 | JFK→EDI | **Nuitée Air** | PENDING_CONFIRMATION | Paid |
> | FH-267-5WE9PCGF | JFK→LHR | **Nuitée Air** | CONFIRMED | Paid |
> | FH-267-21JQSLMK | JFK→LHR | **Nuitée Air** | PENDING_CONFIRMATION | Paid |
> | FH-267-CFYAIKKW | JFK→LHR | **Nuitée Air** | PENDING_CONFIRMATION | Paid |
> | FH-267-1PSW8WQK | AUS→LJU | United Airlines | **FAILED** | Refunded |
> | FH-267-WRUPEDNY | DFW→DBV | United Airlines | **FAILED** | Refunded |
> | FH-267-FOFASTL2 | LGA→OSL | Air Canada | **FAILED** | Refunded |
> | FH-267-JQM6C3OS | LGA→OSL | Air Canada | **FAILED** | Refunded |
> | FH-267-DVQ3X6KS | MIA→LIS | Air Canada | **FAILED** | Refunded |
>
> **Every synthetic-carrier booking succeeds. Every real-carrier booking fails.
> No exceptions.** Payment state is irrelevant: real carriers fail both with and
> without a completed widget payment, and Nuitée Air succeeds either way.
>
> **`AUS→JFK→MRS` was Nuitée Air, not a real carrier.** That booking was recorded
> here as "real GDS + payment = CONFIRMED" and an entire causal theory was built
> on it. The reservations page lists it as Nuitée Air. There has never been a
> successful real-carrier booking on this account.
>
> **Three things only visible on that page, none of them in any API response:**
> 1. The **operating carrier** — the single fact that decides success. Not in
>    the book response, not in the error, not in the request logs.
> 2. A failed book still **creates a booking record** (`Airline PNR: Pending`,
>    `Status: FAILED`). 55099 is not a lost request; it is a booking that was
>    created and could not be ticketed.
> 3. **`Payment Status: Refunded`.** The charge is taken and given back. Our
>    "nothing has been charged" copy was wrong — see `checkoutErrors.ts`.
>
> **The method error, worth more than the finding:** this section was rewritten
> three times in one day, each time confidently, each time from one run with one
> variable changed and no repeat, and twice from a data point that was silently
> mislabelled. The provider-side view settled it in minutes. When an error is
> opaque, go and look at the provider's own records before theorising.
>
> **Consequence:** real-airline booking cannot be validated before production.
> That is an account/entitlement question for Nuitee, not a code one — the
> request we send is byte-identical on success and failure (verified side by
> side in their request logs).
>
> #### Nuitee's own assistant agrees (2026-07-29)
>
> Run the wrench on a 55099 row → "Fix API Error". Its verdict once given the
...
>
> And its recommended actions, which are now the go-live path for flights:
> 1. **Move the same flow to production** (non-sandbox) once the account is
>    enabled for live flights.
> 2. **Ask LiteAPI support to enable live flight booking/ticketing**, and confirm:
>    - which providers are enabled (GDS / NDC sources),
>    - whether there are carrier/provider restrictions,
>    - whether any additional settings are required for the payment flow.
>
> ⚠️ **It was confidently wrong first.** Its opening analysis claimed we were
> posting the *hotel* booking payload (`prebookId` + `payment`) to the flights
> endpoint and told us to rebuild the request — for a body shape that had
> returned 201 twice that same day. It only corrected after being handed the two
> successful booking refs and the carrier split. Useful tool; treat it as a lead
> and check it against the reservations page, which is data rather than
> inference.
>
> Run 3 in full (2 passengers, `Economy Light`):
> ```
> verify → prebook 019faf04-7559-7631-b650-ed33ffde65ce ($2,650.54, 244 services)
>        → attach 4 services (2 bags + 2 seats) → $2,794.39
>        → txn tr_cts_83B…U9iN → tr_cts_PrJ…vXrw        (re-mint, §8)
>        → hosted widget → returnUrl                     (paid, sandbox)
>        → book TRANSACTION_ID with the RE-MINTED txn
>        → CONFIRMED  bookingRef FH-267-LAPCCKWP
> ```
>
> **Proven across runs 3 and 4** (and unaffected by the withdrawal above):
> multi-passenger booking; attach-services end to end; the §8 re-minted
> `transactionId` accepted at book; and the hosted payment widget working
> against a **flight** prebook's `secretKey`, which `docs/user-payment` never
> documents (it is written for hotels).
>
> **What to send Nuitee**, since this is not diagnosable from the client:
> the failing prebookIds (e.g. `019fb0db-5e68-7a35-8c70-bd887bbabc0a`), the fact
> that their own synthetic carrier books reliably on the same key while real GDS
> content mostly does not, and that a completed Stripe payment through their own
> hosted widget does not change the outcome. Ask what `55099`
> (`"unable to process booking request"`) actually means provider-side, and
> whether real GDS inventory is bookable in sandbox at all.
>
> **CONFIRMED BY TEST, same day.** `ACC_CREDIT_CARD` was tried on a fresh
> prebook and rejected:
> ```
> 400 {"error":{"code":45009,
>      "description":"payment method must be CREDIT, TRANSACTION_ID, or THIRD_PARTY",
>      "key":"bodyRequest.payment.method","message":"invalid format: payment method unsupported"}}
> ```
> §10's enum note therefore still holds, and the prebook's `paymentTypes`
> remains untrustworthy for flights. **Do not re-test `ACC_CREDIT_CARD`.**
>
> With `ACC_CREDIT_CARD` eliminated and `CREDIT` requiring a credit line this
> account does not have, **`TRANSACTION_ID` is the only route** — and as of
> 2026-07-29 it is a route that completes a booking on a **real carrier**, in
> sandbox, with the hosted payment widget in front of it. Nothing in the booking
> code is blocked: search → verify → journeyKey refresh → prebook → attach
> services → pay → book all work end to end, for 2 passengers, with extras.
>
> `55099` is the booking-stage sibling of the old prebook-stage `53099`, on a
> prebook that returned a valid `transactionId` + `secretKey` and advertised
> `TRANSACTION_ID, ACC_CREDIT_CARD, WALLET`. Same family as the flaky-inventory
> `502 / 53010 "offer is sold out"` already noted in the gap analysis: a
> provider-side failure, not a request defect. Treat any `5xxxx` as
> "provider could not complete it, re-quote and start again" — which is exactly
> what `classifyCheckoutError` does again now that the 55099 special case
> (a permanent "known blocker on our side" card with no retry) has been removed.
>
> **Open, and only answerable in production:** whether real GDS carriers ticket
> at all. Raise the sandbox-cannot-ticket-real-carriers hypothesis with Nuitee
> alongside the §14 cancellation gap.
>
> ### The "settled" conclusion above was wrong. Real carriers DO book — intermittently. (2026-08-09)
>
> Traced from a traveller report during split-payer testing: two devices
> booking the same United Airlines ATL→EWR→NAP flight, one got `55099`, the
> other didn't. The AI diagnostic bundled with Nuitee's dashboard blamed a
> trailing-slash endpoint mismatch and a wrong payment-method enum — both
> checked against our actual source and both false (`API_BASE` has no
> trailing slash, `payment.method` is the literal string `'TRANSACTION_ID'`).
> Same lesson as before: that assistant guesses without seeing the request: do
> not trust it over the dashboard's own reservation records.
>
> The reservations page for that exact session (`connect.nuitee.com/flights/`)
> settles it instead:
>
> | Booking ID | Booked | Fare | Seat | Status |
> |---|---|---|---|---|
> | 019fe794-9fcc | 12:31 | $1,012.43 | 22F, purchased | **PENDING_CONFIRMATION**, Paid |
> | 019fe795-6660 | 12:32 | $1,012.43 | 22D, purchased | FAILED, Refunded |
> | 019fe795-dcb0 | 12:33 | $960.47 | none | FAILED, Refunded |
> | 019fe79a-53a7 | 12:37 | $1,012.43 | 15A, purchased | FAILED, Refunded |
> | 019fe79a-a0e0 | 12:38 | $960.47 | none | FAILED, Refunded |
>
> Same carrier, same route, same session, four attempts within seven minutes —
> one succeeded. Fare and seat selection track which fresh offer each
> re-quote landed on (the app's own "get a fresh price and retry" behavior),
> not success or failure: the succeeding and one failing attempt share the
> identical $1,012.43 fare with a purchased seat one row apart (22F vs 22D).
> Nothing distinguishes the one that worked from the three that didn't.
>
> **So `55099` on a real carrier is not a permanent account-entitlement
> block. It is intermittent** — roughly 1 in 5 in this sample — and the
> "cannot be validated before production" conclusion above, and the identical
> one in `GO_LIVE_LITEAPI.md` §3, no longer hold. What was true from the
> earlier 14-booking sample (every synthetic Nuitée Air fare confirms, real
> carriers sometimes fail) is still true; "always" was the overreach.
>
> Practical effect: none. `classifyCheckoutError` already treats any `5xxxx`
> as "provider couldn't complete it, get a fresh price and retry" rather than
> a dead end, which is the correct response to genuine flakiness — it just
> was believed to be correct for the wrong reason. Worth re-raising with
> Nuitee as "intermittent," not "blocked," since the fix on their end (if
> there is one) is a different kind of question.
...
- ~~No cancellation endpoint documented.~~ **Stale as of 2026-09 — cancellation now exists** (§11a). `DELETE /flights/bookings/{id}` still 404s; the real paths are below.
- Fields that matter after booking: `cancelIntentAt` (set only **while** a cancellation awaits airline confirmation, then gone, so it cannot say who cancelled), `lastRefreshedAt`, `remarks[]` (generic provider comments, no documented schedule-change mapping). There is **no old-vs-new diff** after booking: `changes{}` exists only on `/flights/verify`.
- WhereTo stores the itinerary it last told the traveller about in `flight_orders.itinerary_snapshot` and diffs every refresh against it. See [SERVICING.md](SERVICING.md).

Source: `reference/get_flights-bookings-bookingid`.

### 11a. Cancellation - `GET` then `POST /flights/bookings/{bookingId}/cancellations`

- **`GET` = quote, cancels nothing.** `confidence` (`confirmed`|`estimated`|`heuristic`|`unknown`), `isRefundable`, `isVoidable` (inside the void window), `refund` / `penalty` (`{display{amount,currency}}`, margin applied), `penalties[]`, `tickets[]`, `destination` (`original_payment`|`agency_deposit`|`voucher`|`bsp_settlement`|`manual`|`unknown`), `vouchers[]` (airline credit: `code`, `airline`, `pricing`, `validFrom`, `expiresAt`, `passengerNames`, `notes`), `expiresAt`, `pnr`. `refund` is the **potential maximum, not granted**. Every call hits the airline: never poll it. 409 `49006` not quotable in this state, 409 `49007` a cancel is already in flight.
- **`POST` = cancel, no body.** **202** `status: CONFIRMED` = accepted, not final (booking stays CONFIRMED, `cancelIntentAt` set, idempotent while pending). **200** final `CANCELLED` (fee 0) or `CANCELLED_WITH_CHARGES` (fee > 0) with flat `{bookingId, status, cancellation_fee, refund_amount, currency, destination?, vouchers?}`. 409 `46004` already cancelled, `46005` concurrent, `46006` wrong state / not final. 404 `46003` PNR not found at the provider. Final updates arrive as webhooks.

Source: `reference/get_flights-bookings-bookingid-cancellations`, `reference/post_flights-bookings-bookingid-cancellations`.

### 11b. After booking: services and extra charges

- `GET /flights/bookings/{id}/services`: read-only. Booked seats/bags plus the live catalogue. "Booking additional services on an existing booking is not available yet."
- `POST .../extra-charges/precharges` then `.../extra-charges/charges`: attach a charge line to a confirmed booking and collect it through the same payment widget (`usePaymentSdk: true` returns `transactionId` + `secretKey`; `payment.method` must match the original booking). **Not enabled by default** (403 `40301` until LiteAPI support turns it on). This is the only API route to collect a traveller-requested change cost; there is **no API for changing a flight** itself.

Source: `reference/get_flights-bookings-bookingid-services`, `reference/prechargeflightextracharges`, `reference/chargeflightextracharges`.
...
### 12a. A `5xxxx` is not automatically a provider-side failure (2026-09-08)

§10 concludes "treat any `5xxxx` as provider could not complete it, re-quote
and start again". That is right for most of them and **wrong for the ones that
carry a real `description`**. Observed on prebook:

```
500 {"error":{"code":53099,
     "description":"Passenger 1: travel document expires 2026-10-14, before the
       final flight's departure on 2026-10-15. The document must be valid when
       the airline checks it at check-in.",
     "key":"bodyRequest","message":"unable to process prebook request"}}
```

Same code, same `message`, same HTTP status as a genuine GDS refusal. The only
thing that separates them is `description`, and the app was throwing it away:
the traveller got "It rejected the request on its side (code 53099)" over a
**Get a fresh price and retry** button, on a US→US trip they had no reason to
think involved a passport at all. A re-quote resends the identical passenger
block, so that button could never have worked.

Two rules follow, both now enforced in `src/utils/checkoutErrors.ts`:

1. **`description` and `message` are different fields.** `message` is the
   endpoint's boilerplate and is identical on every `53099`. `description` is
   the specific reason. Never flatten them into one string and then re-parse it
   — `src/utils/providerError.ts` keeps the envelope intact for both providers.
2. **`key: "bodyRequest*"` means they rejected what WE sent**, not their own
   inventory. A traveller-data refusal (document, name, date of birth,
   nationality, head count) is recoverable only by editing a field, so it gets
   its own `traveller` issue kind and an action that opens that passenger's
   form. Note the passenger index in `description` is 1-based over the array in
   the order it was sent.

The matching client-side gap: expiry was validated against the OUTBOUND
departure only, so a document lapsing mid-trip passed our form and failed at
LiteAPI. It is validated against the **last flight's departure** now.
...
| Post-book events | order webhooks | 8 `flight.*` webhook events, none for a schedule change | done: re-read + diff on every event, plus a sweep (SERVICING.md) |
| Cancellation | `order_cancellations` | quote + cancel endpoints (§11a) | done: traveller self-serve, quote first (SERVICING.md) |
| Currency/FX | - | `pricing.converted` (bool) + `display` block | respect `converted` in UI |

---

## 15. WhereTo integration notes (the plan, and what shipped)

**Built. `activeFlightProvider` in [src/providers/flights/index.ts](src/providers/flights/index.ts) is `LiteApiFlightProvider`, the money path is the `liteapi-flights` edge function, and bookings land in `flight_orders`.** Two things shipped differently from the plan below: search and booking share one edge function (`liteapi-flights`) instead of a separate `liteapi-flight-book`, and search moved server-side straight away rather than staying client-side. The plan is kept because the reasoning behind each point still explains why the code looks the way it does.

The plan as written, against the seam the abstraction gave us:
