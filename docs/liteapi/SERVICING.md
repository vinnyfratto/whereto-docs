...
| Guest support | Three desks, supplied 2026-09-12 in the three-layer requirements Appendix A: `customer-support@nuitee.com` routine, `emergencies@nuitee.com` within 7 days of check-in, `inresort@nuitee.com` on the spot. No response targets are published for any of them. | Us, per Terms §4. We relay to the right desk; the traveller never sees one. |
...
While the trip is under way the section leads with **I'm at the airport and need help now** / **I'm at the property and need help now**, which are the only two topics that change where a case goes rather than what it says. They are offered from the day the trip starts until the day it ends, and the server routes on the same window, so a booking three weeks out can never reach an emergency desk.
...
3. picks the Nuitée desk and the urgency bucket (`flightRelayRoute` / `hotelRelayRoute` in `_shared/bookingChanges.mjs`), from how close the trip is and what the traveller said happened. The buckets and response targets are Nuitée's own, from Appendix A of the three-layer requirements:

   | Flights | Desk | They answer in |
   |---|---|---|
   | At the airport, flying today or mid-trip | `flights.emergency@nuitee.com` | 30 minutes to 1 hour |
   | Departs within 24 hours | `priorityflights@nuitee.com` | 2 to 3 hours |
   | Trip in progress | `flights.inflight@nuitee.com` | 4 to 6 hours |
   | Departs within 7 days | `flights.inflight@nuitee.com` | 4 to 6 hours |
   | 1 to 3 weeks out | `flights@nuitee.com` | 48 hours |
   | Beyond 3 weeks, unknown, or already flown | `flights@nuitee.com` | 72 hours |

   | Hotels | Desk |
   |---|---|
   | At the property, between check-in and check-out | `inresort@nuitee.com` |
   | Check-in within 7 days | `emergencies@nuitee.com` |
   | Everything else | `customer-support@nuitee.com` |

   Two things in Nuitée's table read oddly and are copied deliberately: 24 hours to 1 week goes to `flights.inflight@`, not `flights@`; and the document skips between 3 weeks and 1 month, so anything past 3 weeks takes the slower 72 hour target rather than one Nuitée has not committed to. Nuitée publishes no hotel response targets, so the app quotes none. `operations@nuitee.com` is retired and a test asserts it cannot come back;
4. saves the request to `booking_help_requests`, including the ticket fields (service type, issue type, urgency, travel date, route) in `context`;
5. emails **support@wheretotrips.com** (cc `BOOKING_HELP_CC`, reply-to the traveller) with the traveller's message, the facts, a message ready to forward to that desk plus a one-tap mailto, and a **Ticket fields** block of `Key: value` lines (`supportFields` in `_shared/help-emails.ts`) for Zoho Desk to parse into custom fields; and emails the traveller that we have it (reply-to support).
...
**Cancel this flight** / **Cancel this stay** on the booking page opens `app/bookings/cancel.tsx`, which shows the numbers before anything can be cancelled. It is hidden once the booking is no longer active, once the trip has started, and for **flights inside 48 hours of departure**: Nuitée's own servicing document says the airline takes operational control in the final 24 to 48 hours, so a quote we showed there would very likely not hold. That case gets a help request instead, and the cancel screen makes the same check before it spends an airline quote call. Hotels keep self-serve cancel to the last day, because a free-cancellation deadline is often 24 or 48 hours out and `hotelCancellationPreview` already quotes the real fee.
...
9. ~~Why does a cancellation settle about 15% under its own quote?~~ **Answered 2026-09-22.** See §6.
10. ~~`isRefundable` and `isVoidable` are absent from the quote payload rather than false.~~ **Answered 2026-09-22.** See §6.


---

## 6. Cancellation amounts: what LiteAPI told us

Asked 2026-09-22 with two real sandbox cancellations, because the quote and the
result disagreed by a constant factor per booking (0.84575 and 0.84667, applied
to the fee and the refund alike, to five decimals) and it was not obvious which
number a traveller actually gets.

**The POST is authoritative.** `refund_amount` and `cancellation_fee` from
`POST /flights/bookings/{id}/cancellations` are what settles to `destination`.
The GET quote is a preview and its refund is a "potential maximum ... not
granted". So:

- the quote screen shows **"Refund, up to"** and says the airline sets the final
  amount on completion;
- the result screen and the confirmation email report the **POST** figures as
  final, which is what they already did.

**Do not gross the POST figures up.** We had wondered whether the gap was our
margin being stripped, since the quote reconciles to the fare we sold almost to
the cent. LiteAPI's answer: that is not how net-versus-gross would behave, the
same constant would not land on both the fee and the refund, and partners should
not reverse-engineer a factor the API does not define.

**Tested 2026-09-22, and it is definitively not our margin.** Flight commission
was set to 0%, a fresh sandbox flight booked (`FH-269-SRKES9VE`) and cancelled.
At 0% our sale price equals the airline price exactly (`order.price.total` ==
`pricing.subtotal` == 2061.41), the quote still reconciles to the fare, and the
POST still comes back short — by a factor now **identical on both components to
five decimals**:

| Commission | fee ratio | refund ratio |
|---|---|---|
| 8% | 0.84575 | 0.84667 |
| **0%** | **0.85494** | **0.85494** |

Every sandbox cancellation we have ever done, for the record. `POST sum` is
`cancellation_fee + refund_amount`; the quote for each reconciled to the fare.

| Booking | PNR | Fare paid | POST fee | POST refund | POST sum | Ratio | Unaccounted |
|---|---|---|---|---|---|---|---|
| FH-269-Y60V0LM8 | RHIN53 | 3,497.69 | 2,591.21 | 370.17 | 2,961.38 | 0.846667 | 536.31 |
| FH-269-JLB0X2BX | 2PNDYE | 2,797.95 | 2,070.56 | 295.80 | 2,366.36 | 0.845748 | 431.59 |
| FH-269-JY5XME01 | 3TF8DH | 6,313.85 | 4,658.43 | 665.48 | 5,323.91 | 0.843211 | 989.94 |
| FH-269-LP2UELMY | OHBJ9V | 644.12 | 481.08 | 68.72 | 549.80 | 0.853568 | 94.32 |
| FH-269-SRKES9VE | SUD2Z2 | 2,061.41 | 1,542.08 | 220.30 | 1,762.38 | 0.854939 | 299.03 |

All USD, all sandbox, all `CANCELLED_WITH_CHARGES` with
`destination: original_payment`. The ratio is not constant across bookings, so
it is not one published rate; it is a per-booking reduction we cannot derive
from anything the API returns.

Removing our margin did not close the gap; it only removed the small
inconsistency our own rounding was adding on top. What remains is a single flat
~14.5% reduction applied by LiteAPI to a quote that reconciles perfectly.
**Still unresolved, and only a production cancellation will settle it** — the
`-prod` twins refuse `cancel` by design. If production behaves the same way, the
refund we report to a traveller is ~14.5% under the figure the quote showed them.

**`confidence: "confirmed"` is about the quote, not the amount.** It means the
provider returned a firm response rather than a heuristic. It does not promise
the refund. Both of the cancellations above were `confirmed` and both settled
about 15% under, which is exactly what the wording allows.

**A missing `isRefundable` or `isVoidable` means unknown, not false.** Omission
must not be read as false. We pass both through as `null` now, and only an
explicit `false` with no refund quoted will tell a traveller their ticket returns
nothing. Where the airline simply did not say, the screen says that instead.
