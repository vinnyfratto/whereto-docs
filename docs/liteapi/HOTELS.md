...
> WhereTo: done. "My Bookings" reads `hotel_orders` and `flight_orders` and unions them (`src/store/bookingsStore.ts`). Retrieval by our stored `liteapi_booking_id` or by `clientReference` (via `listbookings`) both work.
...
**Pending (from [HOTEL_BOOKING_REFERENCE.md](../HOTEL_BOOKING_REFERENCE.md) §11):**
Items 1 to 5 shipped and were confirmed by a sandbox booking on 2026-09-17. The
payment enum in use is `TRANSACTION_ID`. What is left:
1. Tighten prebook/book field extraction against a live sandbox response (this doc's `data`-wrapped names are authoritative).
2. Cancellation is built but not yet verified on a device, and its `booking_servicing` SQL has not been run. See `SERVICING.md`.
