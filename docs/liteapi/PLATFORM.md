...
> **The two production keys are not interchangeable — this cost us a week and a support ticket.**
> The dashboard's API Keys tab lists a **"Production – Private API Key"** (`prod_` + digits) *and* a
> **public key** (`prod_public_…`). Only the **private** one goes in the `X-API-Key` header. The
> public one exists solely as the `PublicKey=` component of HMAC signing (§4) and returns
> **401 `{"error":{"code":401,"message":"unauthorized"}}` on every endpoint** when sent as
> `X-API-Key` — including `/data/*` reference endpoints that need no entitlement at all. That
> failure is indistinguishable from a dead or revoked key, which is exactly how we misread it
> (ticket LAS-2081, opened 2026-08-31, resolved as "works fine, you used the public key").
>
> Before setting the `LITEAPI_KEY` secret, prove the key first:
> ```
> npm run check:liteapi-key
> ```
> with `LITEAPI_TEST_KEY` in the environment (`scripts/check-liteapi-key.mjs`). It reports the
> prefix it detected, then hits reference data, `/hotels/rates`, `/flights/rates` and the booking
> host, and never prints the key in full.
...
> WhereTo status (2026-09-16): `supabase/functions/liteapi-webhook` receives every event, logs it to `liteapi_webhook_events` and dedupes on `event_id`. Every flight booking event and every hotel event that can change a stay re-reads that booking and runs it through `_shared/bookingSync.ts`, which updates `flight_orders` / `hotel_orders` and emails the traveller about cancellations, schedule changes and moved stays. Until 2026-09-16 all eight flight events were log-only, which left 11 rows with the wrong status. Event-by-event map and the responsibility split: [SERVICING.md](SERVICING.md).
>
> Two facts from real payloads: the flight events carry the booking id at the top of `response` (`flight.book.confirmed` / `pending.confirmation` / `failed` send the whole booking object), and `flight.book.cancelled` sends a flat result `{status, currency, bookingId, refund_type, refund_amount, cancellation_fee}` with `request: {bookingId}` and **no field saying who cancelled**. Hotel cancel webhooks only fire in production.
...
- **Open gaps (platform):**
  ...
  2. ~~No webhook function yet.~~ Done: `liteapi-webhook` (shared `authorization` token, stringified `request`/`response` parsed, deduped on `event_id`), acting on booking events since 2026-09-16. See [SERVICING.md](SERVICING.md).
