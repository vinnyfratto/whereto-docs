...
| --- | --- | --- |
| LiteAPI key | `LITEAPI_KEY` Supabase Edge Function secret | No (server-side only since 2026-09-17) |
...
- `.env` is gitignored; `credentials.json` and `google-service-account.json` are
  untracked.
- All travel-provider traffic goes through Edge Functions, so the payment-capable key
  never ships to devices. A key inside an APK is spendable against our account, which is
  why the app calls `liteapi-data` rather than LiteAPI directly. See
...
Card capture and payment are handled by **LiteAPI (Nuitee)**, which is the merchant of
record and the PCI-scoped party; it captures through its own Stripe integration
(Model A, see [liteapi/PAYMENTS.md](liteapi/PAYMENTS.md)). The app does not store card
numbers. Document the exact boundary (what data touches our systems versus Nuitee's)
here before go-live, because anyone assessing the payment flows will scope PCI
carefully. _TODO._
