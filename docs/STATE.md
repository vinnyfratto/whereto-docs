...
Storage, Edge Functions). Flights and hotels both via LiteAPI, the only travel provider
(see [ADR-0009](decisions/0009-liteapi-primary-provider.md)), behind swappable provider
interfaces. Analytics via PostHog. Email via Resend. Push via Expo.
...
- Wander Together (group trips): lobby, invites, roles, per-member vibes. Group trips
  book through the same LiteAPI path as solo trips (`useGroupPackageBooking` into the
  Booking Hub).
...
- Error monitoring: Sentry ([ADR-0011](decisions/0011-sentry-error-monitoring.md)). Live in
  the five `liteapi-*` Edge Functions; the app side ships with build 0.3.892. The other
  Edge Functions are not covered yet.
