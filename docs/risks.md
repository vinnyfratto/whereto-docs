...
| R-15 | Group checkout (`group-checkout`) is the last Duffel dependency; solo bookings moved to LiteAPI | P2 | Open | LiteAPI books against one payment per booking; group checkout settles N participants' payments into one order, which needs a real design decision before it can move off Duffel. See the TODO in `supabase/functions/group-checkout/index.ts`. |
