...
| G-04 | **No error monitoring** in production (Sentry shelved; only PostHog analytics). _Closed 2026-09-14: Sentry in the app and `liteapi-*` Edge Functions, ADR-0011._ | P1 | Days | High (you are blind to prod crashes) |
...
- **Secret hygiene**: `.env` is gitignored, `credentials.json` and the Play service
  account are untracked, and the LiteAPI key runs server-side in Edge Functions
  (the app bundle never holds it). This resolves the old "move the travel-provider key
  server-side" blocker.
