...
> **Update 2026-09-17.** The classes in place today are `LiteApiFlightProvider` and
> `LiteApiProvider`, plus the inactive legacy `PricelineProvider`. `DuffelProvider.ts`
> was deleted with the rest of Duffel
> ([ADR-0009](0009-liteapi-primary-provider.md)). The decision itself held up: retiring a
> vendor touched the provider layer and no screens.
