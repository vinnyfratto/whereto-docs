...
> **Update 2026-09-17.** Duffel is gone: the provider, the `duffel-*` Edge Functions and
> `DUFFEL_API_KEY` were all deleted ([ADR-0009](0009-liteapi-primary-provider.md)). The
> rule above outlived the vendor that prompted it and now covers LiteAPI, whose key lives
> in the `LITEAPI_KEY` secret and is read only inside Edge Functions.
...
- LiteAPI currently uses a client-scoped `EXPO_PUBLIC_LITEAPI_KEY`; revisit whether it
  should also move fully server-side. _Resolved by [ADR-0009](0009-liteapi-primary-provider.md):
  it is already read only inside Edge Functions, despite the `EXPO_PUBLIC_` name._
