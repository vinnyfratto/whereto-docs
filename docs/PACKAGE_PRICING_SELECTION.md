...
_Living doc. Last updated: 2026-09-13._
...
hotel floor      = (budget-low x 0.85) - flight price
```
...
1. Drop any hotel whose total is **more than the remaining budget**, or **less than
   the hotel floor** (a package more than 15% under the traveller's budget-low).
   Those are not eligible for the main pick. If nothing clears the floor, the
   floor is dropped and the pick runs again, so a cheap market still gets a hotel.
...
**The hotel lists follow a "Hotel Budget Flexibility" filter** (Package Details
wizard and the hotel browse sheet, first section of Filter Hotels):

| Option | Shows |
|---|---|
| **Within Budget** (default) | packages from 15% under budget-low to 15% over budget-high (with flex) |
| **Within and Below** | anything up to 15% over budget-high (with flex), however cheap |
| **Ignore Budget** | every hotel the search returned |

The +15% over is list-only: the WhereTo Pick still has to fit the real ceiling, and a hotel in that margin shows its package total in amber. The WhereTo Pick always stays in the list. If no hotel matches the chosen option, the
full list shows rather than an empty screen. The filter only changes which hotels
show; it never re-runs the search or moves the pick. Added 2026-09-13 after a tester
with a $4,000-$6,000 budget saw Super 8 second on a cheapest-first list.
