...
```
  Mobile app (Expo / React Native)
        ...
        |   - flights: LiteAPI (active)
        |   - hotels:  LiteAPI (active), Priceline (legacy, inactive)
        |
        v
  Supabase  ------------------------------------------
        |  Postgres (app data, hotel_orders + flight_orders, groups, vibes)
        |  Auth (Google OAuth, sessions)
        |  Storage (avatars, images)
        |  Edge Functions (Deno/TS)  <----- hold all third-party API keys
        |
        +--> LiteAPI  (flights + hotels: search, offers, prebook, checkout, payment)
        +--> Resend   (transactional email + bounce/complaint webhook)
  ...
  Vibe Engine (content/ranking pipeline, see docs/vibes-engine/ -- feeds a review page
               today, not yet imported into Supabase or wired into search scoring)
```
...
- **Third-party calls run server-side** ([ADR-0003](../decisions/0003-server-side-travel-apis.md)):
  Edge Functions hold the keys. The app bundle never carries a travel-provider secret.
- **LiteAPI as the only travel provider** ([ADR-0009](../decisions/0009-liteapi-primary-provider.md)):
  flights and hotels both run on LiteAPI. The previous flight provider is retired and its
  code and Edge Functions were deleted on 2026-09-17.
...
- `scripts/` content and data tooling (icon codegen, destination seeding, image curation,
  Vibe Engine build/QA scripts).
- `Content/VibesEngine/` the Vibe Engine's curated blogs and destination rankings, one
  folder per region/sub-region. See [../vibes-engine/README.md](../vibes-engine/README.md).
