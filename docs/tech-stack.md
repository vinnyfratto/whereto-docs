...
updated: 2026-08-01 (from `package.json`, `app.json`, `eas.json`)._
...
| Flights | LiteAPI (Nuitee Connect, server-side), behind `src/providers/flights/`. Duffel retired for solo bookings, retained only for Wander Together's group checkout — see [ADR-0003](decisions/0003-server-side-travel-apis.md) and [GO_LIVE_LITEAPI.md](GO_LIVE_LITEAPI.md) |
| Hotels | LiteAPI, behind `src/providers/hotels/` (legacy Priceline provider retained, inactive) |
