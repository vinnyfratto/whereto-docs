...
backlog. Last updated 2026-08-22. **Paste this whole doc into a fresh thread to pick
up email work with zero re-derivation** — it's written to be self-contained.

> **2026-08-22 — every Resend email now renders in code.** The Resend *dashboard
> Templates* are retired. HTML is built by three shared modules under
> `supabase/functions/_shared/` and shipped with the functions. Sections 1–3
> below describe the current world; the dashboard-Template workflow they used to
> describe no longer applies to anything we send.

---

## 1. Two separate systems — don't confuse them

1. **Resend** — our own Resend account; our edge functions call the Resend API
   directly with fully-rendered `html`. The HTML is built in TypeScript from a
   shared design system that lives in the repo (see §2), so a design change is a
   code change plus a function redeploy, with no dashboard step. This covers
   booking confirmations, schedule changes, cancellations, welcome, share, friend
   requests, group-booking notices, and the cart nudge.
2. **Supabase Auth email templates** — a *completely different* system. Supabase's
   ...
   These are still hand-pasted HTML files under
   `email-templates/_Emails Added/_Supabase/`.

Both render as edge-to-edge HTML tables — same visual design, entirely different
plumbing to get content into them.

### Why the dashboard Templates went away

Resend Templates substitute flat `{{{VARIABLE}}}` strings. They cannot loop, so a
booking confirmation could only ever show ONE origin→destination pair — no per-
segment flight numbers, no return leg, no connections. They also could not hide a
section that has no data. Half the emails lived in git and half in a dashboard,
and every restyle meant re-importing HTML by hand into each one. Rendering in code
fixes all of that at once.

## 2. Where the HTML lives

| What | File |
|---|---|
| Design system — logo header, trip strip, hero, dark confirmation band, buttons, section titles, time blocks, price table, detail card, callouts, footer, colour tokens | `supabase/functions/_shared/email-theme.ts` |
| Booking confirmation (flight / hotel / both), schedule change, cancellation | `supabase/functions/_shared/booking-emails.ts` |
| Welcome, share destination, share hotel, friend request sent / invite / accepted | `supabase/functions/_shared/lifecycle-emails.ts` |

Group-booking and cart emails build their own sections from `email-theme.ts`
directly, inside their own functions (`together-notify`, `send-group-invite`,
`nudge-participant`, `cart-inactivity-check`). The editorial destination-blog
share (`share-email`, the rich path taken when an IATA code is supplied) keeps its
own bespoke body — it mirrors the in-app vibe blog — but wears the same header and
footer.

**Rules for anything new:** table-based markup with inline styles only (Gmail and
Outlook strip `<style>` blocks and ignore flexbox), colours from the `C` token
object, and never interpolate user-supplied text without `esc()`.

`email-templates/_Emails Added/_Supabase/` still holds the 12 Supabase auth
templates as HTML, because that system genuinely is copy-paste.

## 3. Current inventory

### Sent through Resend (rendered in code)

| Email | Rendered by | Sent from |
|---|---|---|
| Booking confirmation — flight, hotel, or both | `bookingConfirmationHtml` | `send-notification` (fired by `liteapi-flights` / `liteapi-book`) |
| Airline schedule change | `airlineChangeHtml` | `send-notification` |
| Cancellation | `cancellationHtml` | `send-notification` |
| Welcome / account created | `welcomeHtml` | `welcome-email` |
| Share a destination (plain) | `shareDestinationHtml` | `share-email` |
| Share a destination (editorial vibe blog) | `buildDestinationBlogHtml`, in-function | `share-email` |
| Share a hotel | `shareHotelHtml` | `share-email` |
| Saved Traveler invite, has an account | `travelerInviteHtml` | `traveler-invite` |
| Saved Traveler invite, no account yet | `travelerInviteNewHtml` | `traveler-invite` |
| Saved Traveler invite accepted | `travelerInviteAcceptedHtml` | `traveler-invite` |
| Group booking — invite / confirmed / failed | in `together-notify` | `together-notify` |
| Group trip invite | in `send-group-invite` | `send-group-invite` |
| Participant nudge | in `nudge-participant` | `nudge-participant` |
| Cart inactivity nudge | in `cart-inactivity-check` | `cart-inactivity-check` |
| Website form submission + acknowledgement | in `capture-submission` | `capture-submission` |

### The confirmation layout

Built to the 2026-08-22 mockup, top to bottom: colour logo on white → "You're
going to <City>" with departing/returning dates → full-bleed destination photo →
charcoal band carrying "Flight Confirmation" + airline and the booking reference →
greeting and intro → **View full itinerary** → one block per flight segment (city
-to-city heading, flight number, confirmation, fare type, duration, then Departure
and Arrival with times, dates and full airport labels) → hotel block when there is
a stay → free-cancellation note → **Fare details** (base, taxes, Total Paid) →
**View receipt**.

The per-segment data comes from the app: `prepareFlightCheckout` in
`src/utils/flightBooking.ts` puts `slices` on the booking context, with airport
labels ("Austin, TX (AUS - Austin-Bergstrom Intl.)") resolved there because only
the app has the airport dataset. `liteapi-flights` forwards it verbatim. If an
older client omits it, the email falls back to a single origin→destination block —
nothing breaks, it just shows less.
