...
> **Closed tracker. Added 2026-09-17.** This tracked the June 2026 rebuild of the
> Duffel-era passenger wizard. That whole path is gone: the wizard components, the
> `duffel-*` edge functions and the `orders` table were deleted on 2026-09-17, so the
> **▶ PICK UP HERE** marker and the open checkboxes below are not work to resume. Flights
> book through `FlightPrebookView` and `liteapi-flights` into `flight_orders` today. Read
> [liteapi/FLIGHTS.md](liteapi/FLIGHTS.md) for the live flow. Everything below is left as
