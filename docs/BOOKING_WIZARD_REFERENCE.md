...
> **Historical. Do not build against this. Added 2026-09-17.**
> This describes the Duffel-era four-step `PassengerWizard`, which no longer exists.
> `PassengerWizard`, `ReviewConfirmSheet`, `ExtrasSheet`, `SeatMapView`, `offerExtras.ts`,
> the `duffel-*` edge functions and the `orders` table were all deleted on 2026-09-17.
> Flights now book through `FlightPrebookView` and the `liteapi-flights` edge function
> into `flight_orders`, from the same hub at `app/bookings/passengers.tsx`. For the
> current system read [liteapi/FLIGHTS.md](liteapi/FLIGHTS.md) and
> [HOTEL_BOOKING_REFERENCE.md](HOTEL_BOOKING_REFERENCE.md). Kept because it is the only
> written record of how the wizard behaved, which still explains some of the staging
> columns and copy.
